...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(() => {
    ...
    const snapPoints = pinned.map((st) => {
  ...
  // === EXTRA ANIMATIONS FOR ALL SCENES ===
  const scenesExtra = [
    { id: "scene-a", selector: ".scene-content h2, .scene-content p, .scene-content .divider-line", extraAnim: null },
    { id: "scene-b", selector: ".scene-content h2, .scene-content p, .scene-content .divider-line, .mt-6", extraAnim: ".scattered-polaroid" },
    { id: "scene-c", selector: ".scene-content h2, .scene-content p, .scene-content .divider-line", extraAnim: "#love-letter-envelope" },
    { id: "scene-d", selector: ".scene-content h2, .scene-content p, .scene-content .divider-line, .mt-6", extraAnim: ".polaroid-trigger" }
  ];

  scenesExtra.forEach(sceneInfo => {
    const sceneEl = document.getElementById(sceneInfo.id);
    if (!sceneEl) return;

    // Animate texts (Fade up & Stagger)
    const texts = sceneEl.querySelectorAll(sceneInfo.selector);
    if (texts.length) {
      gsap.fromTo(texts, 
        { y: 40, opacity: 0 },
        {
    ...
    // Animate extra interactive items (Pop in & Stagger)
    if (sceneInfo.extraAnim) {
      const extras = sceneEl.querySelectorAll(sceneInfo.extraAnim);
      if (extras.length) {
        gsap.fromTo(extras,
